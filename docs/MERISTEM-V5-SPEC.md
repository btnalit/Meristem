...
| **P0-c 自持化（检验 H2）** | 机制已落地、timer 未启用、解冻待 owner 决策 | 无人值守一夜，早上 **`ignition-status` 计数 > 0 且在增长**（同一判据，§1.2） |
...
| 版本 | 来源 | 主要变更 |
|---|---|---|
| **v5.23** | **P0-c minimal wave：contract quota / breaker / pulse（timer 未启用）** | ① P2-8 有界契约失败配额：`task_state.derive_task_states` 新增 `contract_failures` 计数器，task 归属的 `cycle` 行 `failure_reason=path_violation` 计入其中并在阈值处粘性 `parked`，不触碰 `semantic_failures`/`mechanism_failures`；`propose_failed`（provider deferral，实测 cycle 47-49 延后而任务最终兑现）在任何 task 层级配额里都不计数 · ② 新增 `substrate/breaker.py`（S6 projection）：loop 级、soil-owned，`futile_streak()` 从台账尾部逆向数**连续** task 归属 `cycle` 行中「未产出候选」（`commit` 为假，与 `failure_reason` 无关）的条数，遇到有真值 `commit` 的行即停（候选即便后续被拒也会重置该计数——拒绝是 task 配额的事，不是 breaker 的事）；`manual_cycle(autonomous=True)` 在 dangling-rollback 检查**之前**先查连续futile拍数，≥ `FUTILE_BEAT_THRESHOLD`（5）即调用 `root.panic.engage()` 上闩、退出码 4（新码，核对过与既有 0/1/2/3 无冲突）· ③ 新增 `substrate/pulse-beat.sh` + `substrate/systemd/meristem-pulse.{service,timer}`（S1 projection）：root-only 单拍 `manual-cycle --autonomous` 包装，`flock` 防重入，webhook 读取沿用 `run-soil.sh` 同款「assignment-only + 拒绝可执行语法」解析纪律但从不 source、从不把 URL 导出进子进程环境；退出码 0/1 正常，2/3/4/未知一律 `systemctl disable --now` 计时器并发一条 webhook 通知（该分支逐步骤容错，`curl`/`systemctl` 失败不得吞掉通知本身），文档 docs/MERISTEM-P0C-RUNBOOK.md 新增 · **本轮机制全部到位但 timer 不随部署启用**——解冻是 owner 另行的显式动作，不在本次改动范围内；H1 状态标签不变 |
| **v5.22** | **五项已验证评审发现落地（P0-1/P0-2/P1-3/P1-4/P2 系列）** | ① `blocked` 由粘性终止态改为**衰减态**：机制故障仍计 `mechanism_failures` 并置 `blocked`，但下一条任务归属的台账行按语义正常重算——下一次健康 `cycle` 回落 `open`/`unfulfilled`；`task_guarded` 守卫集合移除 `blocked`（抽成 `_GUARDED_TASK_STATES` 常量），`parked`/`fulfilled`/`promotion_gated` 仍守；`prompt_over_budget` 属确定性 mechanism failure，衰减下会形成不耗 provider 额度的可见重试环，属设计内 · ② 单一 fault 分类源：`task_state.py` 的 mechanism-failure 集合迁至 `learning_state.MECHANISM_FAILURE_REASONS`，删除从未被生产调用、且已与之语义漂移（`prompt_over_budget`/`worker_error` 分类不一致）的 `classify_attempt()` · ③ `model_gateway_server._serve()` 单连接故障隔离：半开连接（`_recv_json_line` 的 `ValueError`）与客户端提前断开（`sendall` 的 `OSError`/`BrokenPipeError`）不再拖垮整个 accept 循环，改为记录到 stderr 后继续下一个连接 · ④ 新增 `runtime_manifest.bootstrap()` 与 `supervisor bootstrap-manifest` 子命令，补全新 checkout 缺失的最小合法状态链（空台账 · 投影 · 空冻结登记 `{}` · manifest 骨架 + `refresh()`）；manifest 已存在即拒绝，不静默重建 · ⑤ rollback 台账硬化：`rollback_intent`/`rollback_committed` 改走 `soil_state.Ledger.append()`（获得与其余台账写入者相同的 ts/event_id 封套与 symlink 拒绝硬化），封套补 `generation`/`soil_cycle`（`execute_autonomous_rollback` 相应加两个显式参数）；新增 `find_dangling_rollback_intents()`，`manual-cycle` 在所有模式、任何变更之前若发现未配对的 `rollback_intent` 即拒绝启动，恢复路径为 `root_manual` 复核（docs/MERISTEM-LAYER0-ROLLBACK.md 新增一节）· ⑥ 三处小修：`model_gateway._credential_value` 的 `pwd.getpwnam("soil")` 补 `KeyError` 捕获（此前只捕 `OSError`）；删除 `feedback_projection._PUBLIC_KEYS` 死代码；新增 `meristem.SEED_WRITABLE == supervisor._WORKER_WRITABLE` 等值断言 |
