...
| A4 | **多信号并存时的根因判定** —— 输入同时包含一个以上类别的触发线索，分类器必须选择真正导致 gate 阻塞的类别；明确描述为“未超限/无问题”的类别不得获选，不能退化为「哪个词表先长出来就选哪个」的偶然顺序 | internal 每条输入只携带单一类别的线索 | internal 的固定字符串从未测试过否定信号、阻塞信号与类别优先级同时出现时的判定 |
...
A4 的隐藏 oracle 采用**显式语义对照**，不是要求实现者证明一个不可判定的“真实因果”：

1. 输入必须同时给出一个被明确否定的候选信号（例如 under ceiling / no issue）和一个被明确
   指出会阻塞 gate 的信号（例如 over cap / holding up the merge）。
2. expect 固定为阻塞信号对应的类别；被否定的候选类别不得获选。
3. 任何未来实现要声称满足 A4，必须通过 vault 中的原始 A4 case；不能只通过增加一个
   关键词或依赖当前 `KEYWORD_TABLE` 的字典顺序来声称满足。
4. 若未来要扩展多信号语义，必须另加成对控制：交换阻塞/非阻塞断言后 expect 应随之变化；
   两个信号都明确为非阻塞时应返回 `unclassified`。这类控制不写入本文件，也不改变当前
   五条 anchor 的数量。

因此 A4 是一个**可重放的行为契约**：测试输入和 expect 由 vault 冻结，语义规则由本节
公开，具体执行算法仍由 organ 自己实现。
## 4. 为什么这个切分能检出「硬编码 internal 的 5 条 case」
...
断言负例（A3）、断言多信号下的**根因选择**（A4）。**第 2 类变更在这 5 个坐标上的期望表现与
修改前完全一致**（因为它从未触及这些坐标），所以 anchor 分数不动；**第 1 类变更**如果真的
改进了底层匹配能力，则大概率也覆盖了 A1/A2/A5 这类同域改写，并应在 A4 遇到“已否定的
非阻塞信号 + 明确阻塞信号”时选择后者。
...
## 5. 与 internal 的边界重申（不在本文件展开，仅引用）
